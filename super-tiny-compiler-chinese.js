 ...
 * and Code Generation
 *
 * 大多数编译器可以分成三个阶段：解析（Parsing），变换（Transformation）以及代码
 * 生成（Code Generation）
 *
 ...
 *
 * 2. *变换*将对这个抽象的表示做一些处理，让它能做到编译器期望
 *    它做到的事情。
 *
 ...
 * language.
 *
 * 编译器的下一步就是变换。它只是把 AST 拿过来然后对它做一些修改。它可以在同种语言下操
 ...
 * on it.
 *
 * 当变换 AST 的时候我们可以添加、移动、替代这些结点，也可以根据现有的 AST 生成一个全新
 * 的 AST
 *
 ...
 *
 * 编译器的最后一个阶段是代码生成，这个阶段做的事情有时候会和变换（transformation）重叠，
